...
### EXEC-001（MUST）每轮必须输出“可执行性判定”

1. AI 在每次执行/反馈结束时，必须显式给出：`ExecutionMode=NOW` 或 `ExecutionMode=REPORT_ONLY_NEXT`。
...
### 6.5 潜在时延风险清单（必须纳入压测矩阵）

1. `点击搜索按钮 -> 搜索输入框可编辑 -> 软键盘可见`  
风险：焦点请求与转场动画竞态，导致“键盘不弹”或“延迟弹出”。
- 建议阈值：P95 < 350ms，P99 < 600ms，Max < 1000ms。
- 采样要求：同进程重复 >= 200 次，重启 runner 重复 >= 100 次。
2. `空白点击/Close -> 预览Sheet消失 -> 地图层恢复可交互`  
风险：关闭写回分叉或事务堆积导致关闭迟滞。
- 建议阈值：P95 < 400ms，P99 < 800ms，Max < 1000ms。
- 采样要求：同进程重复 >= 200 次。
3. `End确认 -> 顶部导航胶囊消失 -> 顶部区域无白条闪现`  
风险：teardown 动画拖尾与 safe area 回收不同步。
- 建议阈值：P95 < 500ms，P99 < 800ms，Max < 1000ms。
- 采样要求：同进程重复 >= 200 次，建议录屏抽检 >= 30 例。
4. `同一帧内 pin 切换（A->B）-> A 预览收敛 -> B 预览稳定呈现`  
风险：双层并存、先关后开时序抖动导致感知卡顿。
- 建议阈值：P95 < 450ms，P99 < 900ms，Max < 1200ms。
- 采样要求：随机切换序列 >= 300 组（含快速连击场景）。
5. `半Sheet“详情”动作 -> detent 进入 large 并稳定`  
风险：内容重排与 detent 变更同帧造成掉帧与错位。
- 建议阈值：P95 < 450ms，P99 < 900ms。
- 采样要求：连续操作 >= 200 次，记录 hitch 样本比率。

### 6.6 潜在时延问题判定规则（新增）

1. 任一链路出现单样本 `>=1000ms` 视为高优先级缺陷，必须阻断发布候选。
2. 任一链路 P99 连续两轮超阈值，判定为 `Fail`；单轮超阈值判定为 `Conditional Pass` 并要求补测。
3. 对“键盘不弹、Sheet不消失、白条闪现”这类功能性时延缺陷，按“功能失败”处理，不仅按性能告警处理。

## 7. 执行协议（不看代码先落规范）

### 7.0 每轮反馈评估（强制）

1. 每完成一个 Step（Step 1-4）或一次独立测试运行，必须追加“可执行性反馈”。
2. 反馈模板固定为：
...
3. 时延：P50/P95/P99/Max（含 §6.5 潜在时延链路分项结果）。

...
1. 编译门禁：`xcodebuild -scheme YOM -destination "$DEST" build` 必须通过。
2. 数据门禁：`LocalMemoryDataIntegrityTests` 必须通过。
3. Phase 0A 收口门禁：补齐 `memory_detail_title` UI 断言测试并纳入必跑集。
...
| --- | --- | --- | --- | --- |
| DOD-0A-01 | 本地 JSON 可加载且 pin 正常渲染 | 自动化 + 手测 | `LocalMemoryDataIntegrityTests` + Map 首屏点位检查 | PASS（证据 `EVID-TEST-DATA`） |
| DOD-0A-02 | Active 链路可达详情并展示核心字段 | UI 自动化 | 新增 `memory_detail_title` 断言用例 | TODO |
| DOD-0B-01 | 完成体验后归档可持久化 | 单测 + UI 流程 | `ExplorationStore` 持久化测试 + 重启验证 | TODO |
| DOD-0B-02 | 卡片可生成并可保存/分享 | 集成测试 | `CardRenderer` 输出 + 相册权限链路 + 分享弹窗断言 | TODO |
| DOD-0C-01 | Passive 围栏提醒可触发 | 集成测试 | `GeofenceMonitor` + `NotificationOrchestrator` + mock 位置 | TODO |
| DOD-0C-02 | 去重/冷却生效 | 单测 | 冷却窗口 24h 行为测试 | TODO |
| DOD-CONFIG-01 | Always + PhotoAdd 权限键齐全 | 配置核查 | `project.yml` 中 `INFOPLIST_KEY_*` 存在性检查 | TODO |
| DOD-CONFIG-02 | Background App Refresh 不可用时降级链路正确 | UI 自动化 + 断言 | `testBackgroundRefreshUnavailableShowsGuidance`（待实现） | TODO |
| DOD-I18N-01 | 新增 UI 字符串有 en / zhHans / yue 三语覆盖 | 代码审计 | `GATE-I18N`（PR 合入前必跑） | TODO |
...
xcodebuild -scheme YOM -destination "$DEST" \
  -only-testing:YOMUITests/LocalMemoryDataIntegrityTests test
```
3. `GATE-UI-MEMORY-TITLE-TEST`
```bash
xcodebuild -scheme YOM -destination "$DEST" \
  -only-testing:YOMUITests/YOMUITests/testMemoryDetailTitleAssertion test
```
4. `GATE-CONFIG-PLIST-KEYS`
```bash
rg -n "INFOPLIST_KEY_NSLocationAlwaysAndWhenInUseUsageDescription|INFOPLIST_KEY_NSPhotoLibraryAddUsageDescription" project.yml
```
5. `GATE-MODULE-0B0C-IMPLEMENTED`
```bash
rg -n "ExplorationStore|CardRenderer|GeofenceMonitor|NotificationOrchestrator" Features Shared
```
6. `GATE-SNAPSHOT-BASELINE-TESTS`
```bash
xcodebuild -scheme YOM -destination "$DEST" \
  -only-testing:YOMUITests/YOMUITests/testSnapshotBaselineRetrievalPage test
```
7. `GATE-SNAPSHOT-THRESHOLD-CONFIG`
```bash
rg -n "snapshotDiffTolerance: Double = 0.01|snapshotChannelDeltaTolerance: Int = 8" YOMUITests/YOMUITests.swift
```
8. `GATE-BG-REFRESH-GUARD-TEST`（当前应为 FAIL，直到实现）
```bash
xcodebuild -scheme YOM -destination "$DEST" \
  -only-testing:YOMUITests/YOMUITests/testBackgroundRefreshUnavailableShowsGuidance test
```
9. `GATE-OWNER-PLACEHOLDER`（任务卡开工前必跑；命中任务卡 Owner 占位符即 FAIL，未命中即 PASS）
```bash
! rg -q "^\| T-PR[0-9]-[0-9]{2} \|.*@TBD \|" "specs/002-community-memory-backend/社区记忆库后端实装规划报告.md"
```
10. `GATE-I18N`（每个涉及新增 UI 字符串的 PR 必跑；验证 LocalizedStringKey 均在三语 Localizable.strings 中有对应键）
```bash
# 列出 Swift 源码中所有新增 LocalizedStringKey（相对于 main 分支的 diff）
git diff main --unified=0 -- '*.swift' \
  | rg '^\+.*LocalizedStringKey\s*\(\s*"([^"]+)"' -o --replace '$1' \
  | sort -u > /tmp/new_keys.txt
# 验证三语文件均包含这些键
for lang in en zhHans yue; do
  echo "=== $lang ==="; comm -23 /tmp/new_keys.txt \
    <(rg '"([^"]+)"\s*=' "$lang.lproj/Localizable.strings" -o --replace '$1' | sort -u)
done
# 任意语言有输出（缺键）即视为 FAIL
```
11. `GATE-PR1-COMPOSITE`
```bash
xcodebuild -scheme YOM -destination "$DEST" build && \
xcodebuild -scheme YOM -destination "$DEST" \
  -only-testing:YOMUITests/LocalMemoryDataIntegrityTests test && \
...
| --- | --- | --- | --- | --- |
| PR-1 | 先补”可执行基础” | 补权限键配置（Always + PhotoAdd）、补 `memory_detail_title` UI Test、补 Snapshot/CI 门禁映射、补 Background App Refresh 降级门禁 | 无 | `GATE-PR1-COMPOSITE` + `GATE-SNAPSHOT-BASELINE-TESTS` + `GATE-SNAPSHOT-THRESHOLD-CONFIG` + `GATE-BG-REFRESH-GUARD-TEST` + `GATE-OWNER-PLACEHOLDER`（应为 PASS，即任务卡 Owner 已实名且无 `@TBD` 占位） |
| PR-2 | 完成 Phase 0B | 实现 `ExplorationStore` + `CardRenderer` + 档案页最小闭环 | **PR-1 已合入 main** | 体验完成后可持久化，重启可见，卡片可导出；`GATE-BUILD` + ExplorationStore/CardRenderer 测试通过 |
| PR-3 | 完成 Phase 0C | 实现 `GeofenceMonitor` + `NotificationOrchestrator`（Top-N、冷却、去重） | **PR-2 已合入 main**；RISK-06 缓解措施已落地 | Passive 提醒可触发、24h 去重生效、已归档默认不重复提醒；`GATE-BUILD` + 围栏/通知测试通过 |
...
| T-PR2-01 | 实现 `ExplorationStore` | iOS-Core @xiangkaisun | P0 | 2026-03-14 | 2026-03-16 | `Features/Memory/*`（新文件可加） | Map 主流程状态机 | `GATE-BUILD` + 持久化用例（新增后纳入 `xcodebuild test`） | 重启后记录丢失或数据不一致 |
| T-PR2-02 | 实现 `CardRenderer` | iOS-Media @xiangkaisun | P1 | 2026-03-14 | 2026-03-17 | `Features/Memory/*`、必要桥接 helper | 地图/导航交互层 | `GATE-BUILD` + 渲染/分享用例（新增后纳入 `xcodebuild test`） | 卡片生成失败率 > 1%（20 次样本） |
| T-PR3-01 | 实现 `GeofenceMonitor` Top-N | iOS-Location @xiangkaisun | P0 | 2026-03-18 | 2026-03-20 | `Features/Memory/*` | Retrieval/Archive 现有页面结构 | `GATE-BUILD` + 围栏注册/轮换测试（新增后纳入 `xcodebuild test`） | 监控集合频繁抖动，或注册数量超治理阈值 |
| T-PR3-02 | 实现 `NotificationOrchestrator` 去重冷却 | iOS-Notification @xiangkaisun | P0 | 2026-03-18 | 2026-03-21 | `Features/Memory/*` | Onboarding 既有权限文案 | `GATE-BUILD` + 去重冷却测试（新增后纳入 `xcodebuild test`） | 24h 冷却失效或重复提醒 |
...
| T-PR2-01 | T-PR1-{01,02,03,04} 全部合入 | PR-2 在 `main` 分支的干净基础上开始，避免配置缺口污染 ExplorationStore 测试 |
...
| EVID-UI-TITLE-ASSERT | 2026-03-09 23:03 | 详情页标题 UI 断言用例 | `rg -n "memory_detail_title" YOMUITests` | FAIL（预期） | 用例仍待新增（T-PR1-02） |
| EVID-BGREFRESH-GUARD | 2026-03-09 23:03 | Background App Refresh 降级保护用例 | `rg -n "testBackgroundRefreshUnavailableShowsGuidance" YOMUITests/YOMUITests.swift` | FAIL（预期） | 用例仍待新增（T-PR1-04） |
| EVID-CONFIG-KEYS | 2026-03-09 23:03 | Passive/相册权限键 | `rg -n "INFOPLIST_KEY_NSLocationAlwaysAndWhenInUseUsageDescription|INFOPLIST_KEY_NSPhotoLibraryAddUsageDescription" project.yml` | FAIL（预期） | 两键仍待补齐（T-PR1-01） |
| EVID-MODULE-0B0C | 2026-03-09 23:03 | 0B/0C 核心模块实现度 | `rg -n "ExplorationStore|CardRenderer|GeofenceMonitor|NotificationOrchestrator" Features Shared` | FAIL（预期） | 仍为待实现项（PR-2/PR-3） |
